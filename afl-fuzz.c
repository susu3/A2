...
static consistency_collect_mode_t rfc_consistency_mode = COLLECT_DISABLED;
static u32 rfc_consistency_sampling_rate = 10;  // Default 10‰ sampling rate
static u8 rfc_consistency_enabled = 0;
...
static bool is_safe_path(const char *path) {
  ...
  // For absolute paths, do additional checks
  char *resolved_path = realpath(path, NULL);
  if (resolved_path) {
    // Check if the resolved path is within sensitive system directories
    bool is_unsafe = (strstr(resolved_path, "/etc/") == resolved_path ||
                     strstr(resolved_path, "/root/") == resolved_path ||
                     strstr(resolved_path, "/sys/") == resolved_path ||
                     strstr(resolved_path, "/proc/") == resolved_path ||
                     strstr(resolved_path, "/dev/") == resolved_path);
    free(resolved_path);
    return !is_unsafe;
  }
  
  return true; // If realpath fails, allow (file might not exist yet)
}

//===============================================Specification Parsing: Markdown to JSON+TXT+MMD===============================================
void protocol_analysis_and_update(const char *protocol_name, const char *rfc_path, const char *output_dir) {
  char *grammar_prompt = NULL;
  char *grammar_response = NULL;
  char *fsm_prompt = NULL;
  char *fsm_response = NULL;
  bool success = false;
  ...
  if (!protocol_name || !rfc_path || !output_dir) {
      fprintf(stderr, "Error: Invalid input parameters for protocol analysis\n");
      return;
  }

  // Validate paths for security
  if (!is_safe_path(rfc_path)) {
      fprintf(stderr, "Error: RFC path appears to be unsafe: %s\n", rfc_path);
      return;
  }
  
  if (!is_safe_path(output_dir)) {
      fprintf(stderr, "Error: Output directory path appears to be unsafe: %s\n", output_dir);
      return;
  }

  // Check if RFC file exists and is readable
  if (access(rfc_path, R_OK) != 0) {
      fprintf(stderr, "Error: Cannot read RFC file: %s\n", rfc_path);
      return;
  }

  // Phase 1: Initial Protocol Analysis
  ...
  grammar_prompt = construct_prompt_for_message_grammar(protocol_name, rfc_path);
  if (!grammar_prompt) {
  ...
  grammar_response = chat_with_llm(grammar_prompt, 3, 0.5);
  if (!grammar_response) {
  ...
  fsm_prompt = construct_prompt_for_state_machine(protocol_name, rfc_path);
  if (!fsm_prompt) {
  ...
  fsm_response = chat_with_llm(fsm_prompt, 3, 0.5);
  if (!fsm_response) {
  ...
  printf("Parsing and saving state machine...\n");
  ...
  bool grammar_updated = verify_and_update_protocol_grammar(protocol_name, output_dir, rfc_path);
  
  if (grammar_updated) {
      printf("Protocol grammar was verified successfully!\n");
  ...
  bool state_machine_updated = verify_and_update_protocol_state_machine(protocol_name, output_dir, rfc_path);
  
  if (state_machine_updated) {
      printf("Protocol state machine was verified successfully!\n");
...
cleanup:
  ...
  if (grammar_prompt) free(grammar_prompt);
  if (grammar_response) free(grammar_response);
  if (fsm_prompt) free(fsm_prompt);
  if (fsm_response) free(fsm_response);

  if (!success) {
      fprintf(stderr, "Protocol analysis failed - continuing with existing configuration\n");
      return;
  }
  ...
  if (grammar_updated || state_machine_updated) {
      ...
      // Check if new grammar files actually exist before printing
      if (grammar_updated) {
          char *new_json_path = NULL;
          char *new_txt_path = NULL;
          if (asprintf(&new_json_path, "%s/new_%s_message.json", output_dir, protocol_name) != -1 &&
              asprintf(&new_txt_path, "%s/new_%s_message.txt", output_dir, protocol_name) != -1) {
              
              // Check if files actually exist
              bool json_exists = (access(new_json_path, F_OK) == 0);
              bool txt_exists = (access(new_txt_path, F_OK) == 0);
              
              if (json_exists || txt_exists) {
                  printf("Grammar improved:\n");
                  if (json_exists) {
                      printf("  %s: Updated protocol grammar (JSON)\n", new_json_path);
                  }
                  if (txt_exists) {
                      printf("  %s: Updated additional information (TXT)\n", new_txt_path);
                  }
              } else {
                  printf("Grammar verification passed - no improvements needed\n");
              }
          }
          
          if (new_json_path) free(new_json_path);
          if (new_txt_path) free(new_txt_path);
      }
      
      if (state_machine_updated) {
          char *new_mmd_path = NULL;
          if (asprintf(&new_mmd_path, "%s/new_%s_fsm.mmd", output_dir, protocol_name) != -1) {
              
              // Check if file actually exists
              bool mmd_exists = (access(new_mmd_path, F_OK) == 0);
              
              if (mmd_exists) {
                  printf("State machine improved:\n");
                  printf("  %s: Updated state machine (Mermaid)\n", new_mmd_path);
              } else {
                  printf("State machine verification passed - no improvements needed\n");
              }
...
void enrich_initial_seeds()
{
  ...
  char *seeds_prompt = NULL;
  char *seeds_answer = NULL;
  char *sequences_prompt = NULL;
  char *sequences_answer = NULL;
  char **sequences = NULL;
  char **messages = NULL;
  int num_sequences = 0;
  int num_messages = 0;
  ...
  // Validate input parameters
  if (!protocol_name || !out_dir || !in_dir) {
      fprintf(stderr, "Error: Invalid parameters for seeds enrichment\n");
      return;
  }
  ...
  seeds_answer = chat_with_llm(seeds_prompt, GRAMMAR_RETRIES, 0.5);
  if(seeds_answer == NULL) {
    fprintf(stderr, "Failed to get message seeds from LLM\n");
    goto cleanup;
  }
  printf("Seeds answer: %s\n", seeds_answer);

  //Extract sequences from the LLM output
  sequences = extract_sequences(seeds_answer, &num_sequences);

  if (sequences != NULL && num_sequences > 0) {
    printf("Extracted %d message seeds from LLM output\n", num_sequences);
    // Write the extracted sequences to seed files
    write_sequences_to_seeds(seedfile_path, sequences, num_sequences);

    // Free the allocated memory for sequences (using ck_free for ck_alloc)
    for (int i = 0; i < num_sequences; i++) {
      if (sequences[i]) ck_free(sequences[i]);
    }
    ck_free(sequences);
    sequences = NULL;
  } else {
    WARNF("No valid message seeds extracted from LLM output");
  }
  ...
  sequences_answer = chat_with_llm(sequences_prompt, GRAMMAR_RETRIES, 0.5);
  if(sequences_answer == NULL) {
    fprintf(stderr, "Failed to get sequence seeds from LLM\n");
    goto cleanup;
  }
  printf("Sequences answer: %s\n", sequences_answer);

  //Extract messages from the LLM output
  messages = extract_messages(sequences_answer, &num_messages);

  if (messages != NULL && num_messages > 0) {
    printf("Extracted %d sequence seeds from LLM output\n", num_messages);
    ...
    WARNF("No valid sequence seeds extracted from LLM output");
  }
...
cleanup:
  // Clean up allocated memory
  if (seeds_prompt) free(seeds_prompt);
  if (seeds_answer) free(seeds_answer);
  if (sequences_prompt) free(sequences_prompt);
  if (sequences_answer) free(sequences_answer);
  
  // Clean up arrays if not already freed
  if (sequences) {
    for (int i = 0; i < num_sequences; i++) {
      if (sequences[i]) ck_free(sequences[i]);
    }
  ...
  if (success) {
    printf("\nInitial seeds generation completed successfully!\n");
  } else {
    fprintf(stderr, "Seeds enrichment failed - continuing with existing seeds\n");
  }
  printf("=================================\n");
}
...
void initial_seeds_optimization(){
  ...
  printf("\nOptimizing initial seeds\n");
  printf("=================================\n");
    
    if (!in_dir) {
        WARNF("Input directory not set, skipping seed optimization");
    return;
  }
    ...
    if (seed_count <= 1) {
        ACTF("Too few seeds (%d) for optimization, skipping afl-cmin", seed_count > 0 ? seed_count - 2 : 0);
        if (seed_count > 0) {
    ...
    ACTF("Optimizing initial seeds using afl-cmin...");
    ...
    if (asprintf(&temp_dir, "%s_optimized", in_dir_copy) == -1) {
        free(in_dir_copy);
        PFATAL("Unable to allocate memory for temp directory path");
    }
    free(in_dir_copy);
    ...
    if (mkdir(temp_dir, 0700) && errno != EEXIST) {
        PFATAL("Unable to create temp directory '%s'", temp_dir);
    }
    ...
    if (asprintf(&cmd_prefix, "afl-cmin -i '%s' -o '%s' -m %llu -t %u%s%s --", 
        in_dir,           // input directory
        temp_dir,         // output directory  
        (unsigned long long)mem_limit,        // memory limit
        exec_tmout,       // timeout
        qemu_mode ? " -Q" : "",  // QEMU mode if enabled
        getenv("AFL_FORKSRV_INIT_TMOUT") ? " -f" : ""  // fork server timeout
    ) == -1) {
        PFATAL("Unable to allocate memory for afl-cmin command prefix");
    }
    
    // Construct target command line from use_argv
    char *target_cmdline = NULL;
    u32 target_len = 1; // for null terminator
    
    // Calculate total length needed
    for (int i = 0; use_argv[i]; i++) {
        target_len += strlen(use_argv[i]) + 1; // +1 for space
    }
    
    target_cmdline = ck_alloc(target_len);
    target_cmdline[0] = '\0';
    
    // Build target command line
    for (int i = 0; use_argv[i]; i++) {
        if (i > 0) strcat(target_cmdline, " ");
        strcat(target_cmdline, use_argv[i]);
    }
    
    if (asprintf(&cmin_cmd, "%s %s", cmd_prefix, target_cmdline) == -1) {
        free(cmd_prefix);
        ck_free(target_cmdline);
        PFATAL("Unable to allocate memory for full afl-cmin command");
    }
    ...
    if (ret != 0) {
        WARNF("afl-cmin failed (exit code: %d), keeping original seeds\n\n", ret);
    ...
    ACTF("afl-cmin completed successfully");
    ...
    ACTF("Original seeds: %d, Optimized seeds: %d", original_seeds, optimized_seeds);
    
    if (optimized_seeds <= 0) {
        WARNF("afl-cmin produced no seeds, keeping original seeds");
        // Clean up
        if (opt_count > 0) {
    ...
    if (asprintf(&backup_dir, "%s_backup", in_dir_backup) == -1) {
        free(in_dir_backup);
        PFATAL("Unable to allocate memory for backup directory path");
    }
    free(in_dir_backup);
    ...
    if (system(backup_cmd) != 0) {
        WARNF("Failed to backup original seeds");
    } else {
        ACTF("Successfully backed up original seeds to %s", backup_dir);
    }
    ...
    if (system(copy_cmd) != 0) {
        WARNF("Failed to copy optimized seeds");
        // Try to restore backup
        snprintf(copy_cmd, sizeof(copy_cmd), "cp '%s'/* '%s'/ 2>/dev/null", backup_dir, in_dir);
        if (system(copy_cmd) != 0) {
            FATAL("Failed to restore backup! Manual recovery needed. Original seeds backup: %s, Optimized seeds: %s", backup_dir, temp_dir);
        } else {
            ACTF("Restored original seeds from backup");
        }
    } else {
        ACTF("Seed optimization completed: %d → %d seeds (%.1f%% reduction)", 
...
static u8 save_if_interesting(char** argv, void* mem, u32 len, u8 fault) {
  ...
  if (fault == crash_mode) {
    ...
    /* RFC Consistency Analysis: Collect interaction data if enabled */
    hnb = has_new_bits(virgin_bits);
    // if (rfc_consistency_enabled && should_collect_consistency_data(hnb)) {
    //   collect_interaction_data(kl_messages, response_buf, response_bytes, messages_sent);
    // }
...
static void usage(u8* argv0) {
  ...
  SAYF("\n%s [ options ] -- /path/to/fuzzed_app [ ... ]\n\n"
       ...
       "  -r file       - protocol specification file\n"
       "  -Y mode       - protocol consistency analysis mode (0: disable collection; 1: only new coverage; 2: new coverage + no duplicate random sampling)\n"
       "  -Z rate       - protocol consistency analysis sampling rate (‰)\n"
...
int main(int argc, char** argv) {
  ...
  while ((opt = getopt(argc, argv, "+i:o:f:m:t:T:dnCB:S:M:x:QN:D:W:w:e:P:KEq:s:RFc:l:b:h:r:Y:Z:")) > 0)
    ...
    switch (opt) {
      ...
      case 'Y': /* RFC consistency analysis mode */
        rfc_consistency_enabled = 1;
        if (optarg) {
          int mode = atoi(optarg);
          if (mode < 0 || mode > 2) {
            FATAL("Invalid RFC consistency mode (0=disabled, 1=new coverage only, 2=with sampling)");
          }
          rfc_consistency_mode = (consistency_collect_mode_t)mode;
        } else {
          rfc_consistency_mode = COLLECT_NEW_COVERAGE_ONLY;
        }
        break;
  
      case 'Z': /* RFC consistency sampling rate */
        if (!optarg) FATAL("Missing sampling rate for -Z");
        rfc_consistency_sampling_rate = atoi(optarg);
        if (rfc_consistency_sampling_rate > 1000) {
          FATAL("RFC consistency sampling rate must be 0-1000 (per mille)");
        }
        break;
  ...
  if (rfc_consistency_enabled && !protocol_selected) {
    FATAL("RFC consistency analysis (-Y) requires protocol selection (-P)");
  }
  ...
  if (protocol_selected)
  {
    ...
    // if (asprintf(&parse_result_path, "%s/%s-parse-result", out_dir, protocol_name) == -1) {
    //     PFATAL("Unable to allocate memory for parse result path");
    // }
    // protocol_analysis_and_update(protocol_name, rfc_path, parse_result_path);
    // free(parse_result_path);

    enrich_initial_seeds();
    // After enriching seeds, optimize them using afl-cmin
    if (qemu_mode)
    ...
    //   // Check prerequisites for RFC consistency analysis
    //   if (!protocol_name) {
    //     WARNF("RFC consistency analysis requires protocol name, disabling");
    //     rfc_consistency_enabled = 0;
    //   } else if (!rfc_path) {
    //     WARNF("RFC consistency analysis requires RFC specification file (-r), disabling");
    //     rfc_consistency_enabled = 0;
    //   } else if (init_rfc_consistency_analysis(out_dir) == 0) {
    //     set_rfc_consistency_mode(rfc_consistency_mode, rfc_consistency_sampling_rate);
    //     ACTF("RFC consistency analysis enabled (mode=%d, sampling=%u‰)", 
    //         rfc_consistency_mode, rfc_consistency_sampling_rate);
    //     ACTF("Protocol: %s, RFC file: %s", protocol_name, rfc_path);
    //   } else {
    //     WARNF("Failed to initialize RFC consistency analysis, continuing without it");
    //     rfc_consistency_enabled = 0;
    //   }
    // }
  }
  ...
  // Set use_argv for non-protocol mode (protocol mode already set it above)
  if (!protocol_selected) {
    if (qemu_mode)
      use_argv = get_qemu_argv(argv[0], argv + optind, argc - optind);
    else
